...
const RuleEditorScreen = () => {
  const [rules, setRules] = useState([
    { id: 1, sensor: 'TEMP', condition: '>=', value: 30, action: 'servo', actionValue: 20 },
    { id: 2, sensor: 'NH3', condition: '>=', value: 2, action: 'led', actionValue: 'on' },
  ]);
    ...
    <ScrollView style={{ padding: 16 }}>
      ...
      {rules.map((rule) => (
        <View key={rule.id} style={{ borderWidth: 1, borderRadius: 8, padding: 12, marginBottom: 16 }}>
          ...
          >
            <Picker.Item label="온도 (TEMP)" value="TEMP" />
            <Picker.Item label="습도 (HUMI)" value="HUMI" />
            ...
            <Picker.Item label="톨루엔 (TOLUENE)" value="TOLUENE" />
          ...
          >
            <Picker.Item label="서보모터 각도 변경" value="servo" />
            <Picker.Item label="LED 제어" value="led" />
          </Picker>
          ...
          {rule.action === 'servo' ? (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 8 }}>
              <Button
                title="-"
                onPress={() =>
                  updateRule(rule.id, 'actionValue', Math.max(0, rule.actionValue - 5))
                }
              />
              <TextInput
                style={{
              ...
              <Button
                title="+"
                onPress={() =>
                  updateRule(rule.id, 'actionValue', Math.min(180, rule.actionValue + 5))
                }
              />
            </View>
            <Text>현재 각도: {rule.actionValue}°</Text>
          </View>
          ) : (
            <View>
              <Text>LED 상태</Text>
